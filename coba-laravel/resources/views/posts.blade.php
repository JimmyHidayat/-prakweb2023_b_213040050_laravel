    ...
    @if ($posts->count())
        <div class="card mb-3">
            ...
            <div class="card-body text-center">
                ...
                <p><small class="text-muted">By. <a href="/authors/{{ $$post[0]->author->username }}"
                            class="text-decoration-none">{{ $$post[0]->author->name }}</a><a
                            href="/categories/{{ $$post[0]->category->slug }}"
        ...
        <div class="row">
            @foreach ($posts->skip(1) as $post)
                <div class="col-md-4 mb-3">
                    <div class="card">
                        <div class="position-absolute bg-dark px-3 py-2" style="background-color:rgba(0,0,0.7)"><a href="/categories/{{ $$post->category->slug }}" class="text-white text-decoration-none">{{$post->category->name}}</a></div>
                        <img src="https://source.unplash.com/500x400?{{ $post->category->name }}" class="card-img-top"
                            alt="{{ $post->category->name }}">
                        <div class="card-body">
                            <h5 class="card-title">{{ $post->title }}</h5>
                            <p><small class="text-muted">By. <a href="/authors/{{ $$post->author->username }}"
